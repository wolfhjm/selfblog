...
- [x] 候选提取 payload 采用 ABC + Fogg 基础字段。
- [x] 探索结构追问模式围绕 ABC 展开。
- [x] 实验生成 prompt 改为 Fogg MAP。
- [x] 实验复盘记录失败原因：动机、能力、提示、机会、健康状态。
- [x] 实验表保存 MAP 字段：目标行为、动机、能力、提示、更小版本、完成标准、机会和健康背景。

### V1.2：增加事件链

- [x] 一个来源对话生成一个事件链。
- [x] 事件链内包含多个事件，每个事件有自己的解释、情绪、行为和证据。
- [x] 候选收件箱按事件链和所属事件分组展示。
- [x] 事件级操作：继续追问、事件入库、确认本事件下候选。
